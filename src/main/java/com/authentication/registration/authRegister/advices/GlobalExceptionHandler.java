package com.authentication.registration.authRegister.advices;
...
import com.authentication.registration.authRegister.exceptions.UserNotFoundException;
...
//        System.out.println(ex.getStackTrace());
    ...
    @ExceptionHandler(PunchNotAllowedException.class)
    public ResponseEntity<ApiError> handlePunchNotAllowed(PunchNotAllowedException ex) {
        return ResponseEntity.status(HttpStatus.FORBIDDEN)
                .body(new ApiError(ex.getMessage(),403,LocalDateTime.now(),null));
    }
